...
//클래스 상속 관련 처리를 해준다.
$lj.extendsClass = function(childClass, parentClass)
{
...
//----------------------------------------------------------------------------
$lj.requestData = function(jparam)//url, postData, success, error
{
    $.ajax(
    {
        type: 'POST',
...
//해제시 사용
$lj.wndList = new Array();

//fb.ui.Window 팝업창을 닫는다.
$lj.windowClose = function(id)
{
var gwnd = null;
var length = $lj.wndList.length;
...
$lj.pushWindowList = function(id, wnd)
{
var gwnd = null;
var length = $lj.wndList.length;
...
//--------------------------------------------------------------------------------------
$lj.Window = function(id, contentsUrl, isElement)
{
	this.id = id;
...
$lj.Window.prototype =
{
	...
	{
		//$lj.wndList.push(this);
		if(!$lj.pushWindowList(this.id, this)) return;
		...
		*/
		...
		if(!zIndex) zIndex  = 1000 + $lj.wndList.length*100;
...
//-----------------------------------------------------------------------------------------
$lj.Button = function(btnId, width, height)
{
...
//
//-----------------------------------------------------------------------------------------
$lj.ItemList = function(containerId, arrangeType)
{
...
$lj.ItemList.prototype =
{
  //---------------------------------------------------------------------------------------------------------------
  //  containerId 내부의 리스트 아이템 템플릿을 복사하여 완성된 리스트 항목을 만들고
  //  서버로 부터 수신된 데이터를 콜백으로 넘겨 준다.
  //  jparam : arrangeType, callback, itemCount, isAppend
  ...
  createListItems: function(itemCount, isAppend)
  {
      //아이템 가로 스크롤 배치
      ...
      //추가할 아이템의 시작과 끝 인덱스
      var start = curListItems.length - 1;
...
//--------------------------------------------------------------------------------
$lj.Accordion = function(menuClass, contentsClass, menuHeight)
{
  this.menu = $('.'+menuClass);
...
$lj.Accordion.prototype = 
{
  //speed(에니메이션 속도), isAnimation, isShowToggle(펼쳐진 항목 다시 클릭시 숨길지), showEvent(bind event name)
  setAccordionOption: function(jOption)
  {
      for(var p in jOption)
      { 
          if(jOption[p]!=undefined) this.jOption[p] = jOption[p];
      }
  },
  ...
  showHideManage: function(newSel)
  {
      ...
      if(this.selected!=newSel)
      {
          ...
          if(this.jOption.isAnimation) citem.slideDown(this.jOption.speed, function(){
              //컨텐츠 height 변경시 iframe 외부의 height를 맞춰줌
              parent.popupBodyResize($('body'));
          });
          else citem.show();
      }
      
      //현재 펼쳐져서 보여지고 있는 아이템이 있으면 숨긴다.
      if(this.selected)
      {
          citem = $(this.selected).next();
          
          if(this.jOption.isAnimation) citem.slideUp(this.jOption.speed, function(){
              //컨텐츠 height 변경시 iframe 외부의 height를 맞춰줌
              parent.popupBodyResize($('body'));
          });
          else citem.hide();
          
          if(this.selected==newSel) newSel = null;
      }
  ...
  {
      this.menu.css({'height':this.menuHeight, 'padding-left':this.paddingX,
                                'padding-top':this.paddingY, 'padding-right':this.paddingX});
...
$lj.Tab = function(containerId, nTabHeight, iframe_src)
{
	//수정
	this.src=iframe_src;
	//수정
...
$lj.Tab.prototype = 
{
	...
	applyTab: function()
	{
		...
		{			
            ...
            //수정
          	if(thisTab.src){
          		var iframeTab=thisTab.src+"&tab="+$(tid+' a').index(thisTab.selected);
				//alert(iframeTab);
				$("#iframe_tab").attr("src",iframeTab);
			}
			//수정
...
//-----------------------------------------------------------------------------------------
$lj.Progress = function(prgId, width, height)
{
    this.prgId = prgId;
    this.width = width;
    this.height = height;
