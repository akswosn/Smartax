...
Ext.define("Ext.ux.grid.Printer", {
    ...
    statics: {
        ...
        print: function(grid, featureId) {
            ...
            var expanderTemplate, pluginsBodyMarkup = [];
            ...
            var htmlMarkup = [
                ...
                '<html class="' + Ext.baseCSSPrefix + 'ux-grid-printer">',
                  ...
                  '</div>',
                  '<div style="text-align:center;font-size:20px;font-weight:bold;margin-bottom:10px;">' + this.mainTitle + '</div>',
                  '<div style="text-align:center;font-size:10px;margin-bottom:5px;">' + this.subDate + '</div>',
                    '<table>',
                      '<tr>',
                        headings,
                      '</tr>',
                ...
                '</html>',
                {
                    ...
                    renderColumn: function(column, value, rcd, col)
                    {
                    	...
                    	else if (column.renderer)
                    	{
                    		...
                    		if(value && column.xtype == 'numbercolumn')
                    		{
                    			if((value+'').indexOf(',')>-1) value = Ext.util.Format.number(value.replace(/,/g, ''), '0,000') ;	
                    			else value = Ext.util.Format.number(value, '0,000') ;
                    		} 
                    	}
...
//        },
        ...
        {
            var groups   = null;
            if(grid.store.getGroups) groups   = grid.store.getGroups();
