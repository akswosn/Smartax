											...
											items: [
												{
								               		...
								               		cls:'bottomChild',
								               		handler : function(){
						               					var date = Ext.getCmp('menu07_date').getValue();
                                                        var year = date.getFullYear();
                                                        var month = date.getMonth() + 1;
														
								               			Ext.ux.grid.Printer.mainTitle = '[ 재무상태표 ]';
								               			Ext.ux.grid.Printer.subDate = '( '+year+'.01 ~ '+year+'.'+month+' )'; 
...
//***************************************  컨트롤러 ***************************************//
    ...
    onContainerAfterRender: function(component, eOpts) {
    	var now = new Date();
		Ext.getCmp('menu07_date').setValue(new Date());
    ...
    onMenu07_SearchBtnClick: function(button, e, eOpts) {
        ...
        var date = Ext.getCmp('menu07_date').getValue();
        var to_yyyymmdd = date.getFullYear();
        to_yyyymmdd += month = Ext.String.leftPad((date.getMonth()+1), 2, '0');
        to_yyyymmdd += Ext.String.leftPad(date.getDate(), 2, '0');
    	
        Global.showMask(Ext.getBody());
        
        Ext.Ajax.request({
            method: 'POST',
            url: './proc/account/report/balance_sheet_adv_proc.php',
            params:{
            ...
            success: function(response, opts) {
                ...
                if (json.CODE == '00') thisObj.settingStore(json);
            },
