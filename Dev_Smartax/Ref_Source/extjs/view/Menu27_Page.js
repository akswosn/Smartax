            ...
            items: [
                {
                    ...
                    items: [
                        ...
                        },
                        {
                            ...
                            listeners: {
                                click: {
                                    fn: me.onMenu27_DescriptBtnClick,
                                    scope: me
                                }
                            }
                        }
...
//***************************************  컨트롤러 ***************************************//
    ...
    onContainerAfterRender: function(component, eOpts) {
		...
		var now = new Date();
		Ext.getCmp('menu27_date_str').setValue(Ext.Date.getFirstDateOfMonth( now ));
		Ext.getCmp('menu27_date_end').setValue(Ext.Date.getLastDateOfMonth( now ));
		
		this.onMenu27_SearchBtnClick();
    ...
    //조회버튼을 눌렀을 경우
    onMenu27_SearchBtnClick: function(button, e, eOpts) {
    	
    	var thisObj = this;
    	var fromDt = Ext.getCmp('menu27_date_str').getValue();
		var toDt = Ext.getCmp('menu27_date_end').getValue();
		...
		Ext.Ajax.request({
			...
			success: function(response, opts) {
				...
				if(json.CODE == '00'){
					...
					StoreInfo.Menu27_Grid.removeAll();
					StoreInfo.Menu27_Grid.add(json.DATA);
				}
				else{
					Ext.Msg.alert("", '조회 실패');
				}
    ...
    //물음표버튼을 눌렀을 경우
    onMenu27_DescriptBtnClick: function(button, e, eOpts) {
    ...
    onDeleteBtnClick: function(button, e, eOpts) {
    	...
    	var thisObj = this;
    	var store = StoreInfo.Menu27_Grid;
    	var delRec = Ext.getCmp('Menu27_Grid').getSelectionModel().getSelection()[0];
    	if(delRec)
    	{
	            ...
	            if(btn=='yes'){
					...
					Ext.Ajax.request({
						...
						success: function(response, opts) {
							...
							var json = Ext.JSON.decode(response.responseText);
							if(json.CODE == '00'){
								StoreInfo.Menu27_Grid.remove(delRec);
								thisObj.down('form').getForm().reset(true);
								thisObj.selRec = null;
							}
							else{
