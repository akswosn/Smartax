									...
									items: [
										{
						               		...
						               		cls:'bottomChild',
						               		handler : function(){
						               			Ext.ux.grid.Printer.mainTitle = '[ 영농작목코드 ]';
...
//***************************************  컨트롤러 ***************************************//
    ...
    onContainerAfterRender: function(component, eOpts) {
    	...
    	Ext.getCmp('workjakmok_reg').add(this.form);
    	
  	 	this.addBtnClick();
    },
    
    addBtnClick: function(button, e, eOpts) {
    	this.form.resetData(Define.WJAK);
    },
    ...
    onRegisterCode: function(button, e, eOpts){
		...
		{
			if(this.form.isModifyCheck(Define.WJAK)){
    			Ext.MessageBox.confirm('경고!', '중복된 코드입니다. 작성한 정보로 업데이트 하시겠습니까?', function(btn){
		            if(btn=='yes'){
		            	thisObj.form.onUpdateCode(Define.WJAK);
		            }
		        });
    		}
    		else if(thisObj.form.isModifyNameCheck(Define.WJAK))
    		{
    			Ext.MessageBox.confirm('경고!', '이미추가된 작목명입니다. 추가로 저장 하시겠습니까?', function(btn){
		            if(btn=='yes'){
		            	thisObj.form.onRegisterCode(Define.WJAK);
		            }
		        });
    		}
    		else this.form.onRegisterCode(Define.WJAK);
		}
		else this.form.onUpdateCode(Define.WJAK); 
    },
    
    onDeleteCode: function(button, e, eOpts){
    	//0:작목코드 1:영농작목코드
    	this.form.onDeleteCode(Define.WJAK);
    },
