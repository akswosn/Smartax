...
Ext.define('Common_Pop_Trds_Search', {
    ...
    setCustomerGrid: function(dataArr)
    {
    	...
    	StoreInfo.Menu09_Grid_SEARCH.removeAll();
    	StoreInfo.Menu09_Grid_SEARCH.add(dataArr);
    	
    	//필터링
		StoreInfo.Menu09_Grid_SEARCH.filterBy(function(rec){
			var tmp = null;
			if(thisObj.customer_SKey){
				if(thisObj.customer_StrKey == thisObj.customer_SKey)
				{
					tmp = (Number(rec.get('customer_id'))+'').substring(0,1);
		    		if(tmp == thisObj.customer_SKey) return true;
			    	else return false;
		    	}
		    	else
		    	{
		    		if(rec.get('customer_id').indexOf(thisObj.customer_StrKey) != -1)  return true;
		    		else return false;
		    	}
		    }
		    return true;
    ...
    onGridpanelItemDblClick: function(dataview, record, item, index, e, eOpts) {
    	...
    	else
    	{
	    	...
	    	{
	    		this.record.set(this.dataIndex, record.get('customer_id'));
	    		Global.isEnter = true;	
	    	}
    	}
    ...
    onSelectClick: function() {
    	...
    	else
    	{
	    	...
	    	{
	    	    this.record.set(this.dataIndex, selectData.customer_id);
	    		Global.isEnter = true;
	    	}
    	}
