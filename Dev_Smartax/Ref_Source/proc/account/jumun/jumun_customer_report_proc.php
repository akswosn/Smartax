<?php
	...
	try
	{
		...
		$res = $jWork->requestCustomerReport();
		...
		while($item=$jWork->fetchMapRow()){
			...
			if($customer_id!=-1 and $customer_id!=$item['jumun_tr_cd']){
				//거래처계 넣기 type -1
				$obj=null;
				$obj['jumun_tr_cd']='(거래처 계)';
				$obj['jumun_su']=$su_sum;
				...
				//거래처 갱신
				$customer_id=$item['jumun_tr_cd'];
			}
			
			if($customer_id==-1) 	$customer_id=$item['jumun_tr_cd'];
