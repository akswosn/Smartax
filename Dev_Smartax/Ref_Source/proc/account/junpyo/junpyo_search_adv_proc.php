	...
	try
	{
		...
		if(count($item) > 0)
		{
			while(true)
			{
				...
				$match_cuscode = $item["match_customer_id"];
				if($match_cuscode == 0 or $match_cuscode == '' or $match_cuscode == null) $match_cuscode = '';
				else $match_cuscode = sprintf("%05d",$match_cuscode);
				...
				echo "{ jp_id : '$item[jp_id]' ,  jp_yyyymmdd : '$item[jp_yyyymmdd]' ,  jp_no : '$item[jp_no]', jp_group: '$jp_group' , jp_date_m : $month, jp_date_d: $day,  
						gycode : '$item[gycode]' ,  customer_id : '$cuscode' ,  match_customer_id : '$match_cuscode' ,  jakmok_code : '$jcode', debit : $item[debit] ,  credit : $item[credit] ,
