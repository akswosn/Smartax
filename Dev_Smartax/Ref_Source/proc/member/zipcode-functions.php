...
function get_post_code_xml_by_api($query){
	...
	$post_data = array(
	    'target' => 'post',
	    'regkey' => 'a314996f3c4a87fb21420528889260',
	    'query' => $query
	);
		...
		<tbody>
			<?php 
			    foreach ($xml->itemlist->item as $value) {
			    	...
			    	echo '<tr>';
			    	echo "<td class='address'><a href='#{$value->postcd}'>{$value->address}</a></td>";
			    	echo "<td class='postcd'><a href='#{$value->postcd}'>{$postcd}</a></td>";
			    	echo '</tr>';
			    }
			?>
		</tbody>
