 ...
 * 매출자료입력 메뉴 컨트롤러
 */
Ext.define('Smartax.controller.supertax.TaxSalesController',{
	extend: 'Ext.app.Controller',
	id:'taxSalesController',
    
    views: ['supertax.TaxSalesMain', 'PageView'],
//    store : [],
//    
    refs : [
           {
        	   ref : 'taxSalesMain',
        	   selector : 'taxSalesMain'
           },
           {
        	   ref : 'pageView',
        	   selector : 'pageView'
...
//					this.onSalesCheckLoad();
					this.onSalesInvoiceLoad();
				},
			...
			//탭 변경 이벤트등록
			'taxSalesMain > [id=taxTab]' : {
				tabchange : function(tabpanel, newCard, oldCard, eOpts){
					var thisObj = this.getTaxSalesMain();
					var me = this;
					//tab에 따라 그리드를 교체
					if(newCard.id == 'salesCheckTab'){
						thisObj.thisSelectTabId = 'salesCheckTab';
						me.onSalesCheckLoad();
						
						var ifr = $('#salesCheckFrameView').contents();
						if(ifr[0].baseURI == 'about:blank'){
							setTimeout(function(){
								ifr = $('#salesCheckFrameView').contents();
						...
						thisObj.thisSelectTabId = 'salesInvoiceTab';
						me.onSalesInvoiceLoad();
						var ifr = $('#salesInvoiceFrameView').contents();
						me.onDefaultFormLoad(ifr);
					}
				}
			},
			//검색버튼 이벤트 등록
			'taxSalesMain >[cls=searchBar] > [id=searchBtn]' : {
				click : function(){
					this.onGridViewSearch(Ext.getCmp('search_year').value, Ext.getCmp('search_type').value);
				}
			},
			'taxSalesMain > [id=salesGridLay] > [id=sales_Grid]' :{
				 itemclick: function(dataview, record, item, index, e, eOpts){
            		 this.onGridPanelItemClick(dataview, record, item, index, e, eOpts);
            	 }
			},
			//초기화 버튼 이벤트 등록
//			'taxSalesMain > [id=taxTab] > [xtype=toolbar] >  [id=formResetBtn]' :{
//				click : function(){
//					this.onFormReset();
//				}
//			},
			//등록
			'taxSalesMain > [id=taxTab] > [xtype=toolbar] > [id=formSavetBtn]' :{
				click : function(){
					this.addTaxInfomation();
				}
			},
			'taxSalesMain > [id=taxTab] > [xtype=toolbar] > [id=formDelBtn]' :{
				click : function(){
...
//    	
	...
	onSubPage : function(){
		...
		var me = this;
		thisObj.thisSelectTabId = 'salesInvoiceTab';
		Ext.getCmp('salesCheckTab').add(thisObj.salesCheckPage); 
		Ext.getCmp('salesInvoiceTab').add(thisObj.salesInvoicePage); 
		
		//render bug로 인해 setimeout 설정(iframe의 경우 render전 생성시 화면이 깨짐 ㅠ)
		var ifr = $('#salesInvoiceFrameView').contents();
		if(ifr[0] == null || ifr[0].baseURI == 'about:blank' ){
			setTimeout(function(){
				ifr = $('#salesInvoiceFrameView').contents();
				me.onDefaultFormLoad(ifr);
			},500);
		}
		else {
    ...
    onSalesCheckLoad : function(){
		...
		//console.log(thisObj.searchGridStore);
    	//ajax 조회
    	if(thisObj.salesCheckStore == null || thisObj.salesCheckGridStore == null){
    		thisObj.salesCheckStore = Ext.create('Smartax.store.supertax.TaxListStore');
    		thisObj.salesCheckGridStore = Ext.create('Smartax.store.supertax.TaxDetailStore');
    		...
    		Ext.Ajax.request({
    			...
    			success: function(response, opts) {
    				...
    				if(json.CODE == '00'){
    					thisObj.salesCheckStore.removeAll();
    					thisObj.salesCheckGridStore.removeAll();
    					
    					var data = json.DATA;
    					//grid store 생성
    					thisObj.salesCheckStore.add(data);
    					...
    					thisObj.salesCheckGridStore.add(gridData);
//    					controller.onLoadTaxListGrid(thisObj.salesCheckGridStore.data.items);
    		...
    		//그리드 생성 호출
    		var data = thisObj.salesCheckGridStore.data.items;
    		var year = Ext.getCmp('search_year').value;
			var type = Ext.getCmp('search_type').value;
    		var dataArr = [];
			for(var i=0; i<data.length; i++)
	 ...
	 * salesInvoiceGrid
	 * salesInvoiceStore
	 * salesInvoiceGridStore
	 * */
    onSalesInvoiceLoad : function(){
    	var thisObj = this.getTaxSalesMain();
    	var controller = this;
    	...
    	//ajax 조회
    	if(thisObj.salesInvoiceStore == null || thisObj.salesInvoiceGridStore == null){
    		thisObj.salesInvoiceStore = Ext.create('Smartax.store.supertax.TaxListStore');
    		thisObj.salesInvoiceGridStore = Ext.create('Smartax.store.supertax.TaxDetailStore');
    		...
    		Ext.Ajax.request({
    			...
    			success: function(response, opts) {
    				...
    				if(json.CODE == '00'){
    					thisObj.salesInvoiceGridStore.removeAll();
    					var data = json.DATA;
    					var gridData =  new Array();
    					thisObj.salesInvoiceStore.add(data);
    					...
    					thisObj.salesInvoiceGridStore.add(gridData);
    		...
    		//그리드 생성 호출
    		var data = thisObj.salesInvoiceGridStore.data.items;
    		//console.log(data);
    		var year = Ext.getCmp('search_year').value;
			var type = Ext.getCmp('search_type').value;
    		var dataArr = [];
...
//    		controller.onLoadTaxListGrid(data);
//    		controller.onGridViewSearch(Ext.getCmp('search_year').value, Ext.getCmp('search_type').value);
//    		controller.onLoadTaxListGrid(data);
    	}
    },
    ...
    onLoadTaxListGrid : function(dataArr){
    	console.log(dataArr);
    	var viewObj = this.getTaxSalesMain();
//		//리스트 그리드 구성
    	if(viewObj.searchGridStore != null){
    		viewObj.searchGridStore.destroy();
    		viewObj.searchGridStore = null;
    	}
    	...
    	Ext.getCmp('salesGridLay').removeAll();
    	
    	if(viewObj.salesGrid != null){
    		viewObj.salesGrid.destroy();
    		viewObj.salesGrid = null;
    	}
    	viewObj.salesGrid = Ext.create('Ext.grid.Panel', {
    		cls:'grid',
    		id:'sales_Grid',
    		title:'입력리스트',
    		width:'100%',
...
//                    		 scope: this.getTaxSalesMain()
//                    	 }
//                     }
    	});
    	Ext.getCmp('salesGridLay').add(viewObj.salesGrid);
    },
    ...
    onDefaultFormLoad : function(ifr){
    	var thisObj = this.getTaxSalesMain();
    	var data;
    	if(thisObj.thisSelectTabId == 'salesInvoiceTab'){//세금계산서
    		data = thisObj.salesInvoiceStore.data.items[0].data;
    	}
    	else {
    		data = thisObj.salesCheckStore.data.items[0].data;
    	}
    	
    	//공급자 회사정보 바인딩[디폴트 값 설정]
    	ifr.find('#co_saup_no_view').html(data.co_saup_no);
    ...
    onGridViewSearch : function(year, type){
    	...
    	if(thisObj.thisSelectTabId == 'salesInvoiceTab'){
    		defaultGridStore = thisObj.salesInvoiceGridStore;
    		if(defaultGridStore.getCount() == 0){//기존데이터가 없는 경우 리턴
    			return;
    		}
    	}
    	else {
    		defaultGridStore = thisObj.salesCheckGridStore;
    		if(defaultGridStore.getCount() == 0){//기존데이터가 없는 경우 리턴
    			return;
    		}
    	}
    ...
    onGridPanelItemClick : function(dataview, record, item, index, e, eOpts){
    	...
    	if(thisObj.thisSelectTabId == 'salesInvoiceTab'){
    		headerStore = thisObj.salesInvoiceStore;
    		detailStore =  thisObj.salesInvoiceGridStore;
    		ifr = $('#salesInvoiceFrameView').contents();
    	}
    	else {
    		headerStore = thisObj.salesCheckStore;
    		detailStore =  thisObj.salesCheckGridStore;
    		ifr = $('#salesCheckFrameView').contents();
    	}
		...
		Ext.Ajax.request({
			...
			success: function(response, opts) {
				...
				if(json.CODE == '00'){
					...
					//html 폼으로 인해 스토어 교체 처리 불가 ㅠ 스토어 리로드~
					controller.onDefaultFormLoad(ifr);
//					var headerStore, detailStore;
					if(thisObj.thisSelectTabId == 'salesInvoiceTab'){
			    		thisObj.salesInvoiceStore.removeAll();
			    		thisObj.salesInvoiceGridStore.removeAll();
			    		thisObj.salesInvoiceStore=null;
			    		thisObj.salesInvoiceGridStore=null;
			    		controller.onSalesInvoiceLoad();
			    	}
			    	else {
			    		thisObj.salesCheckStore.removeAll();
			    		thisObj.salesCheckGridStore.removeAll();
			    		thisObj.salesCheckStore=null;
			    		thisObj.salesCheckGridStore=null;
			    		controller.onSalesCheckLoad();
			    	}
    ...
    onDeleteTaxInfomation : function(){
    	...
    	if(thisObj.thisSelectTabId == 'salesInvoiceTab'){
    		ifr = $('#salesInvoiceFrameView').contents();
    		listStore = thisObj.salesInvoiceStore;
    		detailStore = thisObj.salesInvoiceGridStore;
    	}
    	else {
    		ifr = $('#salesCheckFrameView').contents();
    		listStore = thisObj.salesCheckStore;
    		detailStore = thisObj.salesCheckGridStore;
    	}
