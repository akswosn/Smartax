...
// 	$sheetIndex->getStyle('A9')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	...
	//-->>>>>>>>>>> 내용 Contents
// 	$sheetIndex->setCellValue('A1','매출 자료입력');
// 	$sheetIndex->setCellValue('F1','1.3');
	
// 	$sheetIndex->setCellValue('A2','Ⅰ. 공급자 인적사항');
...
// 	$sheetIndex->setCellValue('A6','Ⅱ. 매출 부가세 거래내역');
...
// 	$sheetIndex->setCellValue('K8', "기본계정 (대 변)");
// 	$sheetIndex->setCellValue('L8', "상대계정 (차 변)");
// 	$sheetIndex->setCellValue('M8', "영수/청구");
// 	$sheetIndex->setCellValue('N8', "대표자 (15자리)");
// 	$sheetIndex->setCellValue('O8', "업 태 (20자리)");
// 	$sheetIndex->setCellValue('P8', "종 목 (30자리)");
// 	$sheetIndex->setCellValue('Q8', "사업장주소 (80자리)");
// 	$sheetIndex->setCellValue('R8', "비고 (30자리)");
// 	$sheetIndex->setCellValue('S8', "전자");
// 	$sheetIndex->setCellValue('T8', "부서/사원 코드");
// 	$sheetIndex->setCellValue('U8', "현장코드");
// 	$sheetIndex->setCellValue('V8', "PJT코드");
// 	$sheetIndex->setCellValue('W8', "불공제사유");
...
// 	$sheetIndex->setCellValue('A9', "합계");
	...
	for($i=0;$i < count($res);$i++){
		...
		$sheetIndex->setCellValue('N'.(10+$i), $obj['tr_daepyo']);
		$sheetIndex->setCellValue('O'.(10+$i), $obj['tr_up']);
		$sheetIndex->setCellValue('P'.(10+$i), $obj['tr_jong']);
		$sheetIndex->setCellValue('Q'.(10+$i), $obj['tr_addr']);
		$sheetIndex->setCellValue('R'.(10+$i), $obj['tr_bigo']);
		$sheetIndex->setCellValue('S'.(10+$i), "");
